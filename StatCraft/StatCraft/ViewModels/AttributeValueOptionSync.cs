...
namespace StatCraft.ViewModels
{
    ...
    // InsertValueOption(int, string)/DeleteValueOption(int, string) shape, but the two repositories
    ...
    {
        public static void Apply(NotifyCollectionChangedEventArgs e, int attributeId,
            Action<int, string> insertOption, Action<int, string> deleteOption)
        {
            if (e.NewItems != null)
                foreach (string value in e.NewItems.OfType<string>())
                    insertOption(attributeId, value);
